    ...
    $("#ano_id").change(function(){
        ...
        if(parseInt(ano_id)>0){
            $.post(url,{ano_id:ano_id},function(data){
                $("#ano_id").val(data);
                location.href="aplicacion.php?aplicacion="+$("#aplicacion").val()+"&apli_com="+$("#apli_com").val()+"&ano_id="+$("#ano_id").val();
            });
        ...
        if ( (exitoLetras) && (exitoNumeros) )
        {
            ...
            bootbox.confirm("<br><br><b>&iquest;Desea grabar los datos?</b>", function(result) {
                if(result)
                {
                    ...
                    $("#opc").val(1);
                    $("#random").val(random);
                    if(String(valor) === "guardarEje" || String(valor) === "guardarArea" || String(valor) === "guardarPrograma"){
	                    $.post(url,{random:random,idSec:idValue,nombre:$("#nomCatalogo").val(),eje_id:idCatalogo,idTmp:idTmp,active:$("#edoId").val(),ponderacion:ponderacion,opc:opc},function(bufferAct){
...
/****elimina una actividad de la vista  **/
    	...
    	if(parseInt(idProyectoTmp) > 0  && parseInt(idActividad) >0 && parseInt(idTable) > 0){
    		bootbox.confirm("<br><br><b>&iquest;Desear actualizar la informaci&oacute;n de la actividad?</b>", function(result) {
                if(result){
                    ...
                    comenT1  	 = $("#comen1-"+idActividad).code();
                    comenT2  	 = $("#comen2-"+idActividad).code();
                    comenT3  	 = $("#comen3-"+idActividad).code();
                    comenT4  	 = $("#comen4-"+idActividad).code();    
                    random= Math.round(Math.random() * 1000);
    ...
    /******** ACTUALIZAR TABLA PRINCIPAL, TOMANDO REGISTROS DE VISTAS ******/  
    $(document).on('click','#actualizaBasePrimaria', function (){
        $("#procesando").hide();
        $("#t_procesando").hide();
        $("#exito").hide();
        $("#fallo").hide();
    	...
    	if(parseInt(idProyectoTmp) > 0 && parseInt(idTable) > 0){
    		$("#div_procesando").show();
    		bootbox.confirm("<br><br><b>&iquest;Desear actualizar la informaci&oacute;n del proyecto en la base Principal?<br>No olvide realizar su <b>RESPALDO</b> de informaci&oacute;n.</b>", function(result) {
                if(result){
                    $("#procesando").show();
                    $("#t_procesando").show();                	
                	inputNombreAct = $("#inputNombreAct").val();
                	...
                	presupuestoc_1 = $("#presupuestoc_1").val();
                	estimadoc_1 = $("#estimadoc_1").val();
                	$.post(url,{random:random,idTable:idTable,id:idProyectoTmp,proyecto:inputNombreAct,ponderacion:ponderacion,presupuesto:presupuestoc_1,estimado:estimadoc_1,opc:6},function(buffer){                		
                		$("#nom").html(inputNombreAct);
                        $("#procesando").hide();
                        $("#t_procesando").hide();
                		if(parseInt(buffer) == 1){
                			$("#exito").html("Se ha actualizado el proyecto en la base primaria");
                	        $("#exito").show();
                		}else{
                			$("#exito").html("Error al actualizar el proyecto en la base primaria");
                	        $("#fallo").show();
                		}
                		setTimeout(function(){$("#exito").hide();$("#fallo").hide();},1500);
                	});
    ...
    $(document).on('click','.actualizarBasetAct', function (){
        $("#procesando").hide();
        $("#t_procesando").hide();
        $("#exito").hide();
        $("#fallo").hide();    	
    	...
    	idProyectoTmp = $("#idProyectoTmp").val();
    	if(parseInt(idProyectoTmp) > 0  && parseInt(idActividad) >0 && parseInt(idTable) > 0){
    		bootbox.confirm("<br><br><b>&iquest;Desear actualizar la informaci&oacute;n de la actividad en la tabla Princiapl?<br>No olvide realizar su <b>RESPALDO</b> de informaci&oacute;n.</b>", function(result) {
                if(result){
                	$("#procesando").show();
                    $("#t_procesando").show();
                	vTrimestre1  = $("#m1-"+idActividad).val();
                    ...
                    $.post(url,{random:random,idTable:idTable,id:idProyectoTmp,idActividad:idActividad,
                    	...
                    	v1:vTrimestre1,v2:vTrimestre2,v3:vTrimestre3,v4:vTrimestre4,
                    	a1:aTrimestre1,a2:aTrimestre2,a3:aTrimestre3,a4:aTrimestre4,
                    	c1:comenT1,c2:comenT2,c3:comenT3,c4:comenT4,opc:7},function(buffer){                    		
                    	$("#nmAct").html(actividad);
                        $("#procesando").hide();
                        $("#t_procesando").hide();
                		if(parseInt(buffer) == 1){
    ...
    /******** actualizar base principal de manera masiva  *******/
    $(document).on('click','.cargar', function (){
        ...
        if(parseInt(id)>0){
        	 random= Math.round(Math.random() * 1000);
        	url = "ajax/registraActualizacion.php";
        	bootbox.confirm("<br><br><b>&iquest;Antes de actualizar la tabla principal, realiza un <b>RESPALDO</b> de la informaci&oacute;n.", function(result) {
                if(result){
                    $("#procesando").show();
                    $("#t_procesando").show();
                	 $.post(url,{random:random,idTable:id,opc:8},function(buffer){                    		
                         $("#procesando").hide();
                         $("#t_procesando").hide();
                         
                 		if(parseInt(buffer) == 1){
                 			$("#resultado").css({ color: "#006600", background: "#ffffff" });
                 			$("#resultado").html("Se ha actualizado la tabla principal.");
                 		}else{
                 			$("#resultado").css({ color: "#ff0000", background: "#ffffff" });
                 			$("#resultado").html("Error al actualizar la tabla principal.");
                 		}
                 		setTimeout(function(){$("#resultado").html("");},1500);
                 	});  	
                }
        	});
        }
