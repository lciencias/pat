// Validate.js
/*
* validacion de java para formularios
* autor victor hugo diaz del capo vicas
*/
...
//inicio del escript u funciones de validación.
...
alert("seleccione un campo del checkbox.");
}
//radioMsgStack
//checkboxMsgStack 	
				emptyCheckBoxStack();
...
if(checkboxMsgStack[0] != null && checkboxMsgStack[0] != "null" && checkboxMsgStack[0] != "")
			...
			alert("Favor de seleccionar el checkbox.");
...
if(anyErrorMsg == false){
alert("Por favor selecione una opccion.");
}
					emptyRadioStack();
...
function _date()
{
	...
	else
	{
		...
		return false;
			...
			if ((yy % 4) == 0) {
				if ((mm == 2) && (dd > 29)) {
						alert("dia no valido en febrero no es año biciesto");
						return false;
				}
			}
			else {
...
function _usPhoneNumber()
{
	...
	{
		if(!fieldMsg){
		alert("inserte el telefono en el siguiente formato (xxx)xxx-xxxx ");
		}else{
			alert(fieldMsg);
		}
		__setFocus();
...
function _mobileNumber()
{
	...
	{
		if(!fieldMsg){
		alert("por favor inserte el numero telefonico en el siguiente formeto(+xx)xxxxxxxxxx or xxxxxxxxxx pattern.");
		}else{
		alert(fieldMsg);
		}
		__setFocus();
...
{
	if(fieldValue!=""){
	var regex = /^[a-zA-Z0-9\ \.\º\#\-]+$/;
	if(!regex.test(fieldValue))
	{
		if(!fieldMsg){
		alert("este campo es solo para datos del domicilio.");
		}else{
			alert(fieldMsg);
		}
		__setFocus();
...
function _username()
{
	...
	if(!regex.test(fieldValue))
	{
		...
		if(!fieldMsg){
		alert("no utilice caracteres especiales.");
		}else{
			alert(fieldMsg);
		}
		__setFocus();
...
regex2 = /^[\d]+$/;
    ...
    if(!regex1.test(fieldValue) && !regex2.test(fieldValue)) {
		if(!fieldMsg){
		alert("solo ingrese numeros enteros");
		}else{
			alert(fieldMsg);
		}
		__setFocus();
...
if(fieldType == "text" || fieldType == "textarea" || fieldType == "file" || fieldType == "password")
{
	...
	if(fieldRequired.toLowerCase() == "yes" || fieldRequired.toLowerCase() == "required" || fieldRequired.toLowerCase() == "true")
	{
		...
		{
			alert("ESTE CAMPO ES REQUERIDO \ NO SE PUEDE DEJAR VACIO.");
			field.focus();
			return false;
		}
...
if(fieldType == "select-one")
{
	...
	if(trim(fieldValue) == "" && fieldRequired.toLowerCase() == "yes"){
			alert("ESTE CAMPO ES REQUERIDO \ SELECCIONE UNA OPCION.");
		field.focus();
		return false;
	}
	}
