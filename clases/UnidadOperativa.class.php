...
class UnidadOperativa  extends Comunes implements InterfazCatalogos{
    ...
    function regresaDatos(){
        ...
        $sql="SELECT area_id FROM cat_area_unidad_operativa WHERE ".$this->campo."='".$this->data['id']."';";
        $res=$this->db->sql_query($sql) or die($this->cadena_error);
        if($this->db->sql_numrows($res)>0)
        {
    ...
    function actualizar(){
        ...
        if(count($this->data) > 0)
        {
            ...
            {
                if( (trim($clave) != "opc") && (trim($clave) != "random") && (trim($clave) != "idSec") && (trim($clave) != "idarea") && (trim($clave) != "eje_id")  &&  (trim($clave) != "idTmp") )
                {
                    $update[]= $clave." = \"".$valor."\"";
                }
            }
            ...
            if($res){
                ...
                if(count($array_areas) > 0){
                    $sqlE="DELETE FROM cat_area_unidad_operativa WHERE unidad_id='".$folio."';";
                    $resE=$this->db->sql_query($sqlE) or die($this->cadena_error);
                    foreach($array_areas as $idArea)
                    {
                        if( ($idArea + 0) > 0)
                        {
                           $insPol="INSERT INTO  cat_area_unidad_operativa(unidad_id,area_id) VALUES ('".$folio."','".$idArea."');";
    ...
    function formato(){
        $buffer="
            ...
            <tr><td>".AREA."</td><td>".$this->generaAreas('',$this->array_datos['area_id'],1)."</td></tr>
    ...
    function listado(){
        ...
        <table width='95%' ><tr><td class='tdright'><button class='btn btn-success' name='buttonBuscar' id='buttonBuscar' type='button' value='1' onclick=\"location='aplicacion.php?aplicacion=".$this->session['aplicacion']."&apli_com=".$this->session['apli_com']."&opc=1'\">".UNIDADOPERATIVA."</button></td></tr></table>";
        $buf=$imgAct=$imgCon=$imgEli="";
        $arrayActivos = $this->catalogoActivos();
        $sql_count="SELECT distinct(a.".$this->campo.") FROM ".$this->tabla." as a LEFT JOIN cat_area_unidad_operativa as b
                    ON a.unidad_id=b.unidad_id WHERE 1 AND b.unidad_id is  not null ".$this->cadenaFiltros.";";
        ...
        if($no_registros > 0)
        {
            ...
            $sql="SELECT DISTINCT(a.".$this->campo."),a.nombre,a.active,a.orden FROM ".$this->tabla."  as a
                  LEFT JOIN cat_area_unidad_operativa  as b ON a.unidad_id=b.unidad_id
                  WHERE 1 AND b.unidad_id is  not null ".$this->cadenaFiltros." ORDER BY a.area_id asc,a.orden ASC limit ".$this->session['page'].",".$this->session['regs'].";";
               ...
               $buffer.="
                    ...
                    <thead>
                        <tr class='alturaTableHeader'>
                            ...
                            <td width='30%' class='tdcenter cabecerasTable'>".UNIDADOPERATIVA."</td>
                            <td width='50%' class='tdcenter cabecerasTable'>".AREA."</td>
                            <td width='10%' class='tdcenter cabecerasTable'>".ESTATUS."</td>
                            <td width='15%' class='tdcenter cabecerasTable' colspan='3'>".ACCIONES."</td>
                        </tr>
                    </thead>
                    <tbody>";
                $c=0;
                while(list($_id,$_nm,$_act,$_areaId) = $this->db->sql_fetchrow($res))
                {
                    ...
                    $buffer.="<tr class=\"row".(($c++%2)+1)."\">
                        ...
                        <td class='tdleft'>".$this->RegresaAreasUOperativas($_id)."</td>
