...
class GeneraEstadisticas extends ComunesEstadisticas{
	...
	function generaCuadroActividades($arrayTotales){
		...
		foreach($this->arrayDatos as $idAct => $tmpDatos){
			...
			$comentarios      = htmlentities($this->regresaComentarios($arrayComentarios));
			...
			if(trim($comentarios) != ""){
				$this->buffer .='
					...
					<input type="hidden" name="coment-'.$idAct.'" id="coment-'.$idAct.'" value="'.trim($comentarios).'">
					</div>';
			}
            ...
            if(trim($adjuntos) != ""){
				...
				<input type="hidden" name="adjuntos-'.$idAct.'" id="adjuntos-'.$idAct.'" value="'.trim($adjuntos).'"> 
				</div>';
            }            
			$this->buffer .='</td></tr>
