<?php
class Visualizar extends Comunes{
    var $ddb;
    var $data;
    var $path;
    var $session;
   ...
   function inserta(){
		...
		if($this->data['idproyecto'] > 0)
		{
			...
			$sql="SELECT estatus_entrega FROM proyectos_acciones WHERE id='".$this->data['idproyecto']."' limit 1;";
			...
			$ins="INSERT INTO proyectos_validaciones_comentarios (proyecto_id,comentarios,aprobado,fecha_alta)
				  VALUES ('".$this->data['idproyecto']."','".$this->data['content']."','".$this->data['aprobado']."','".$fecha."');";
			$resins=$this->db->sql_query($ins)  or die($this->cadena_error);
			if($resins){
				$this->buffer=$this->db->sql_nextid();
				$update = "UPDATE proyectos_acciones set estatus_entrega='".$statusTo."' WHERE id='".$this->data['idproyecto']."' limit 1;";
				$resupd=$this->db->sql_query($update)  or die($this->cadena_error);
				$ins="INSERT INTO log_estatus (user_id,proyecto_id,ip,estatus_from,estatus_to) VALUES ('".$this->session['userId']."','".$this->data['idproyecto']."','".$this->session['ip']."','".$entregaactual."','".$statusTo."');";
				$this->db->sql_query($ins) or die($this->cadena_error);				
				$this->insertaBitacoraComentarios($this->data,$this->session,$this->data['idproyecto'],0,$this->buffer,$this->arrayEstatus['Validar']);
   			}
   		}
   }
