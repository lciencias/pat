<?php
class UnidadPrograma  extends Comunes implements InterfazCatalogos{
    var $ddb;
    var $data;
    var $path;
    var $session;
    ...
    function __construct($db,$data,$session,$path,$pages){
        ...
        $this->pages  = $pages;
        $this->tabla  = "cat_programas";
        $this->campo  = "programa_id";
    ...
    function regresaDatos(){
        ...
        $sqlr="SELECT area_id FROM cat_area_programa WHERE programa_id ='".$this->data['id']."';";
        $resr=$this->db->sql_query($sqlr) or die($this->cadena_error);
        if($this->db->sql_numrows($resr)>0){
        	while(list($id) = $this->db->sql_fetchrow($resr)){
        		$this->array_datosB[]=$id;
        	}
        }
    }
    ...
    function actualizar(){
        ...
        if(count($this->data) > 0)
        {
            ...
            $array_areas=$this->data['eje_id'];
            $politicaId =$this->data['idTmp'];
            ...
            if($res){
                ...
                if(count($array_areas) > 0){
                    $sqlE="DELETE FROM cat_area_programa WHERE programa_id='".$folio."';";
                    $resE=$this->db->sql_query($sqlE) or die($this->cadena_error);
                    $array_tmp=array();
                    foreach($array_areas as $idArea){
                        if( ($idArea + 0) > 0){
                            $sqlins="INSERT INTO cat_area_programa (area_id,programa_id) VALUES ('".$idArea."','".$folio."')";
                            $resins=$this->db->sql_query($sqlins) or die($this->cadena_error);
                            $array_tmp[]=$idArea;
                        }
                    }
                    if(count($arrayAdmins)>0){
                    	$del="DELETE FROM cat_permisos_areas where area_id='".$idArea."' and programa_id ='".$folio."' ;";
                    	//echo "\n".$del;
                    	$this->db->sql_query($del) or die($this->cadena_error);
                    	foreach ($arrayAdmins as $idUser){
                    		$insPer="INSERT INTO cat_permisos_areas (usuario_id,area_id,programa_id) VALUES ('".$idUser."','".$idArea."','".$folio."');";
                    	//echo"\n".$insPer;
                    		$this->db->sql_query($insPer) or die($this->cadena_error);
                    		
                    	}
                    }
                    $sqlP="DELETE FROM cat_politica_programa WHERE programa_id='".$folio."';";
                    $resP=$this->db->sql_query($sqlP) or die($this->cadena_error);
                    $insp="INSERT INTO cat_politica_programa(politica_id,programa_id) VALUES ('".$politicaId."','".$folio."');";
                    $resp=$this->db->sql_query($insp) or die($this->cadena_error);
                    if(count($array_tmp)>0){
                    	$upd="UPDATE cat_programas SET area_id='".implode(',',$array_tmp)."',politica_id='".$politicaId."' WHERE programa_id='".$folio."' LIMIT 1;";
                    	$resPol=$this->db->sql_query($upd) or die($this->cadena_error);
                    }
                }
            }
        }
            ...
            if($res){
                ...
                if(count($array_areas) > 0){
                    foreach($array_areas as $idArea)
                    {
                        if( ($idArea + 0) > 0)
                        {
                            $insPol="INSERT INTO cat_area_programa (area_id,programa_id) VALUES ('".$idArea."','".$folio."');";
                    ...
                    //inserto en cat_politicas_programas
                    $insp="INSERT INTO cat_politica_programa(politica_id,programa_id) VALUES ('".$politicaId."','".$folio."');";
                    $resp=$this->db->sql_query($insp) or die($this->cadena_error);
    ...
    function formato(){
        $buffer="
            ...
            <input type='hidden' name='valueId' id='valueId' value='".($this->array_datos['programa_id'] + 0)."'>
            <table width='80%' align='center' border='0'>
            <tr>
            <td>".BTNNUEVOPROGRAMA."</td>
            <td><input type='text' required='yes' class='form-control validatexto' style='height: 30px;width:300px;'  value='".$this->array_datos['nombre']."' placeholder='".NMPROGRAMA."' maxlength='200' name='nomCatalogo' id='nomCatalogo'>
            ...
            <tr><td>".POLITICAPUBLICA."</td><td>".$this->generaPoliticas(($this->array_datos['politica_id']+0),3)."</td></tr>
            <tr><td>".AREA."</td><td>".$this->generaAreas('',$this->array_datos['area_id'],1)." ".$this->muestraAyuda($ayudas[1])." </td></tr>
        ...
        if($this->opc != 6){
            $buffer.="<button type='button' class='btn btn-success savecatalogo' id='guardarPrograma' name='guardarPrograma'>".GRABARDATOS."</button>&nbsp;&nbsp;&nbsp;";
        }
            $buffer.="
    ...
    function generaFiltros(){
        $bufferFiltros="
            <table width='80%' align='center' border='0'>
              <tr><td class='tdcenter'>".$this->generaAreas('',$this->data['idarea'],2)."</td></tr>
              <tr><td class='tdcenter'>".$this->generaActivos($this->data['estatus'])."</td></tr>
              <tr><td class='tdcenter'><input type='text' class='form-control' style='height: 30px;' name='busqNombre' id='busqNombre' placeholder='".NMPROGRAMA."' value='".$this->data['busqNombre']."'></td></tr>
    ...
    function recuperaFiltros(){
        ...
        if(($this->data['idarea']+0)>0){
            $this->cadenaFiltros.=" AND b.area_id like '%".$this->data['idarea']."%' ";
        }
        if( trim($this->data['edoId'])!= ""){
    ...
    function listado(){
        ...
        <table width='95%' ><tr><td class='tdright'><button class='btn btn-success' name='buttonBuscar' id='buttonBuscar' type='button' value='1' onclick=\"location='aplicacion.php?aplicacion=".$this->session['aplicacion']."&apli_com=".$this->session['apli_com']."&opc=1'\">".BTNNUEVOPROGRAMA."</button></td></tr></table>";
        $buf=$imgAct=$imgCon=$imgEli="";
        $arrayActivos = $this->catalogoActivos();
        
        $sql_count="SELECT distinct(a.".$this->campo.") FROM ".$this->tabla." as a LEFT JOIN cat_area_programa as b
                    ON a.programa_id=b.programa_id WHERE 1 ".$this->cadenaFiltros.";";
        ...
        if($no_registros > 0)
        {
            ...
            $this->pages->paginate();
            $width=0;
      		$arrayPoliticas = $this->catalogoPoliticas();
      		
            $sql="SELECT DISTINCT(a.".$this->campo."),a.nombre,a.active,a.orden,a.politica_id,a.ponderacion FROM ".$this->tabla."  as a
                  LEFT JOIN cat_area_programa as b ON a.programa_id=b.programa_id 
                  WHERE 1 ".$this->cadenaFiltros." ORDER BY a.nombre asc,a.orden ASC limit ".$this->session['page'].",".$this->session['regs'].";";
            $res=$this->db->sql_query($sql) or die($this->cadena_error);
            if($this->db->sql_numrows($res) > 0)
            {
                $buffer.="
                    ...
                    <tbody>";
                $c=0;
                while(list($_id,$_nm,$_act,$ord,$_pol,$_pond) = $this->db->sql_fetchrow($res))
                {
                    ...
                    $buffer.="<tr class=\"row".(($c++%2)+1)."\">
                        ...
                        <td class='tdleft'>".$_pond."</td>
                        <td class='tdleft'>".$arrayPoliticas[$_pol]."</td>
                         <td class='tdleft'>".$this->RegresaAreas($_id)."</td>
                ...
                $buffer.="</tbody><tfoot><tr><td colspan='9' class='tdcenter'><br>".$this->pages->display_jump_menu()."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$this->pages->display_items_per_page($this->session['regs'])."</td></tr></foot></table>";  
            }
            else
            {
