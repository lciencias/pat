<?php
class ComentariosAvances  extends Comunes{
    var $ddb;
    var $data;
    var $path;
    var $session;
    ...
    function __construct($db,$data,$session,$path){
        ...
        $this->tabla  = "proyectos_avances_comentarios";
        $this->campo  = "id";
        $this->nmBoton="guardarComentarioAvance";
   ...
   function inserta(){
		...
		if($this->data['idproyecto'] > 0 && $this->data['idactividad'] > 0){
			$fecha = date("Y-m-d H:i:s");
   			...
   			if($this->db->sql_numrows($res) == 0){
   				$ins="INSERT INTO ". $this->tabla." (user_id,proyecto_id,actividad_id,comentarios,fecha_alta,trimestre_id)
   					  VALUES ('".$this->session['userId']."','".$this->data['idproyecto']."','".$this->data['idactividad']."','".$this->data['content']."','".$fecha."','".$this->data['idtrimestre']."');";
    ...
    function consulta(){
    	$this->buffer="";
    	if($this->data['idproyecto'] > 0 && $this->data['idactividad'] > 0){
            $sql="SELECT trimestre_id,comentarios FROM ". $this->tabla." 
                WHERE proyecto_id='".$this->data['idproyecto']."' AND actividad_id='".$this->data['idactividad']."' 
    		ORDER BY trimestre_id asc;";
            $res=$this->db->sql_query($sql) or die("error");
            if($this->db->sql_numrows($res) > 0){
                while(list($trimestre_id,$comentarios) = $this->db->sql_fetchrow($res)){
                    $this->buffer .= "Trimestre: ".$trimestre_id."<br>".$comentarios."<br>";
                }
            }
    	}        
    }
