...
class GeneraEstadisticas extends ComunesEstadisticas{
    ...
    function generaTablaEjeProgramaProyecto(){
        ...
        $arrayActividades = self::recuperaActividades(3);
        $sql = "select v.id,v.proyecto,v.actividadId,v.area,v.ponderacionActividad,
                ...
                from (
                        ...
                        FROM ".$this->tabla." WHERE eje_id >0 and programa_id > 0 ".$this->filtro." ".$campos[2]."
                    )
        ...
        if($this->db->sql_numrows($res)>0){
            while(list($id,$proyecto,$actividadId,$area,$ponderacion,$promedio) = $this->db->sql_fetchrow($res)){
                ...
                if(!in_array($actividadId, $arrayAct)){
                    ...
                    $this->arrayDatos[$id]['actividades']  = $arrayActividades[$id];
    ...
    function generaTablaEjePrograma(){
        ...
        $campos = $this->regresaTrimestre($this->data['trimestreId']);
        $arrayActividades = self::recuperaActividades(2);
        ...
        if($this->db->sql_numrows($res)>0){
            while(list($programa_id,$programa,$id,$proyecto,$actividadId,$actividad,$ponderacion,$promedio) = $this->db->sql_fetchrow($res)){
                ...
                $this->arrayDatos[$programa_id]['actividades']  = $arrayActividades[$programa_id];
    ...
    function regresaTrimestre($trimestreId){
        ...
        switch($trimestreId){
            ...
            $array[2]=  "and ( (trimestre1 > 0 and Atrimestre1> 0 ) or (trimestre2 > 0 and Atrimestre2> 0 )  or (trimestre3 > 0 and Atrimestre3 > 0 )  or (trimestre4 > 0 and Atrimestre4 > 0 )) ";
                break;
            case 1;
                $array[0] = ",trimestre1,(trimestre1) as programadas, ";
                $array[1] = "Atrimestre1,(Atrimestre1) as realizadas";
                $array[2]=  "and ( (trimestre1 > 0 and Atrimestre1> 0 ) ) ";
                break;
            case 2;
                ...
                $array[2]=  "and ( (trimestre1 > 0 and Atrimestre1> 0 ) or (trimestre2 > 0 and Atrimestre2> 0 ) ) ";
                break;          
            case 3;
                ...
                $array[2]=  "and ( (trimestre1 > 0 and Atrimestre1> 0 ) or (trimestre2 > 0 and Atrimestre2> 0 )  or (trimestre3 > 0 and Atrimestre3 > 0 )) ";             
                break;          
            case 4;
                $array[0] = ",trimestre1,trimestre2,trimestre3,trimestre4,(trimestre1+trimestre2+trimestre3+trimestre4) as programadas, ";
                $array[1] = "Atrimestre1,Atrimestre2,Atrimestre3,Atrimestre4,(Atrimestre1+Atrimestre2+Atrimestre3+Atrimestre4) as realizadas";
    ...
    function recuperaActividades($opc){
    	$array = array();
    	switch($opc){
    		case 1:
    			$sql= "SELECT eje_id,count(eje_id) FROM ".$this->tabla." WHERE eje_id > 0 ".$this->filtro." GROUP BY eje_id ORDER BY eje_id;";
    			break;
    		case 2:
    			$sql= "SELECT programa_id,count(programa_id) FROM ".$this->tabla." WHERE programa_id > 0 ".$this->filtro." GROUP BY programa_id ORDER BY programa_id;";
    			break;
    		case 3:
    			$sql= "SELECT id,count(id) FROM ".$this->tabla." WHERE id > 0 ".$this->filtro." GROUP BY id ORDER BY id;";
    			break;
    		case 4:
    			$sql= "SELECT unidadResponsableId,count(unidadResponsableId) FROM ".$this->tabla." WHERE unidadResponsableId > 0 ".$this->filtro." GROUP BY unidadResponsableId ORDER BY unidadResponsableId;";
    			break;
    	}
    	$res = $this->db->sql_query($sql) or die("Error:  ".$sql);
    	if($this->db->sql_numrows($res)>0){
    		while(list($id,$total) = $this->db->sql_fetchrow($res)){
    			$array[$id] = $total;
    		}
    	}
    	return  $array;	
    ...
    function generaTablaEje(){
        ...
        $campos = $this->regresaTrimestre($this->data['trimestreId']);
        $arrayActividades = self::recuperaActividades(1);
        $sql = "select v.eje_id,v.eje,v.programa_id,v.id,v.actividadId,v.ponderacionActividad,
                ...
                from (
                        ...
                        FROM ".$this->tabla." WHERE eje_id > 0 ".$this->filtro." ".$campos[2]." 
                    ) 
        ...
        if($this->db->sql_numrows($res)>0){
            while(list($eje_id,$eje,$programa_id,$id,$actividadId,$ponderacion,$promedio) = $this->db->sql_fetchrow($res)){
                ...
                $this->arrayDatos[$eje_id]['actividades']  = $arrayActividades[$eje_id];
    ...
    function generaTablaArea(){
        ...
        $this->arrayDatos = $arrayProgr = $arrayProye = array();
        $campos = $this->regresaTrimestre($this->data['trimestreId']);
        $arrayActividades = self::recuperaActividades(4);
        $sql = "select v.unidadResponsableId,v.area,v.programa_id,v.id,v.actividadId,v.ponderacionActividad,
                ...
                from (
                        ...
                        FROM ".$this->tabla." WHERE unidadResponsableId > 0 ".$this->filtro." ".$campos[2]."
                    )
        ...
        if($this->db->sql_numrows($res)>0){
            while(list($area_id,$area,$programa_id,$id,$actividadId,$ponderacion,$promedio) = $this->db->sql_fetchrow($res)){
                ...
                $this->arrayDatos[$area_id]['actividades']  = $arrayActividades[$area_id];
        ...
        switch($tipo){
            case 1:
                ...
                if((int) $this->data['ponderaId'] == 1){
                	$buffer .= '<th class="tdcenter" >Actividades procesadas</th>';
    ...
    function generaCuadroEjeAreaProyecto(){
        ...
        if(count($this->arrayDatos) > 0){
            ...
            foreach($this->arrayDatos as $ind => $tmp){
                ...
                $valorActT= $valorActT +  (int) $tmp['actividades'] + 0;
                ...
                $this->buffer.= '<tr>
                                    ...
                                    <td class="tdcenter">'.number_format($tmp['actividades'],0,'.',',').'</td>';
                if((int) $this->data['ponderaId'] == 1){
                    $this->buffer.= '<td class="tdcenter">'.number_format($tmp['actividad_id'],0,'.',',').'</td>
                    				 <td class="tdcenter">'.number_format($tmp['ponderacion'],0,'.',',').'</td>
                                     <td class="tdcenter">'.number_format($tmp['promedio'],2,'.',',').'</td>';
                }
                $this->buffer.= '<td class="tdright '.$this->color($rand).'">'.$rand. '%</td>
                                </tr>';               
                 
                $this->xml.="<set label='".$contador."' tooltext='".$tmp['proyecto']." - Actividades: ".number_format($tmp['actividad_id'],0,'.',',')."'  value='".number_format($tmp['actividades'],0,'.',',')."' showvalue='1' link='".$urlTmp."'/>";
            ...
            $promedioTotal = 0;
            if($valorPr > 0){
                $promedioTotal = ($valorPP / $valorPr)*100;
            ...
            $this->buffer.= '</tbody>
                            <thead>
                                <tr>
                                    ...
                                    <th class="tdcenter">'.number_format($valorActT,0,'.',',').'</th>';
            if((int) $this->data['ponderaId'] == 1){
                $this->buffer.= '<th class="tdcenter">'.number_format($valorAct,0,'.',',').'</th>
    ...
    function generaCuadroEjePrograma(){
        ...
        if(count($this->arrayDatos) > 0){
            ...
            foreach($this->arrayDatos as $ind => $tmp){
                ...
                $this->xml.="<set label='".$contador."' tooltext='".$this->catalogoB[$ind]." - Actividades: ".number_format($tmp['actividad_id'],0,'.',',')."' value='".number_format($tmp['actividades'],0,'.',',')."' showvalue='1' link='".$urlTmp."' />";
            ...
            $this->bufferXml .='</tbody>
                            <thead>
                                <tr>
                                    ...
                                    <th class="tdcenter">'.number_format($valorActT,0,'.',',').'</th>';
            if((int) $this->data['ponderaId'] == 1){
                $this->bufferXml.= '<th class="tdcenter">'.number_format($valorAct,0,'.',',').'</th>
                					<th class="tdcenter">'.number_format($valorPP,0,'.',',').'</th>
                                    <th class="tdcenter">'.number_format($valorPr,2,'.',',').'</th>';
            }           
        ...
        if(count($this->arrayDatos) > 0){
            ...
            foreach($this->arrayDatos as $ind => $tmp){
                ...
                $this->bufferXml .='<tr>
                                    ...
                                    <td class="tdcenter">'.number_format($tmp['actividades'],0,'.',',').'</td>';
                if((int) $this->data['ponderaId'] == 1){                	
                    $this->bufferXml.= '<td class="tdcenter">'.number_format($tmp['actividad_id'],0,'.',',').'</td>
                    					<td class="tdcenter">'.number_format($tmp['ponderacion'],0,'.',',').'</td>
                                        <td class="tdcenter">'.number_format($tmp['promedio'],2,'.',',').'</td>';
                }
                $this->bufferXml.= '<td class="tdright">'.$rand. '%</td></tr>';
                 
                $this->xml.="<set label='".$contador."' tooltext='".$this->catalogoA[$ind]." - Actividades: ".number_format($tmp['actividad_id'],0,'.',',')."' value='".number_format($tmp['actividades'],0,'.',',')."' showvalue='1' link='".$urlTmp."' />";
    ...
    function generaCuadroArea(){
        ...
        if(count($this->arrayDatos) > 0){
            ...
            foreach($this->arrayDatos as $ind => $tmp){
                ...
                $this->xml.="<set label='".$contador."' tooltext='".$this->catalogoC[$ind]." - Actividades: ".number_format($tmp['actividades'],0,'.',',')."'  value='".number_format($tmp['actividades'],0,'.',',')."' showvalue='1' link='".$urlTmp."' />";
    ...
    function calculaPonderacion($tmp){
        ...
        if($tmp['promedio'] > 0){
        	...
        	$rand = (($tmp['ponderacion'] / $tmp['promedio']) *100);
            if($rand > 100){
                $rand = 100;
            }           
        }
