...
class GeneraEstadisticas extends ComunesEstadisticas{
	...
	function generaTablaActividades(){
		...
		$sql = "select actividadId,v.actividad,v.ponderacionActividad,v.tipo_actividad_id,v.medida,
				v.t1,v.t2,v.t3,v.t4,v.t,v.a1,v.a2,v.a3,v.a4,v.a,		
				case
				...
				from (
						SELECT actividadId,actividad,ponderacionActividad,tipo_actividad_id,medida,
						trimestre1 as t1,trimestre2 as t2,trimestre3 as t3,trimestre4 as t4,total as t,
						Atrimestre1 as a1,Atrimestre2 as a2,Atrimestre3 as a3,Atrimestre4 as a4,totalAvance as a						
						".$campos[0].$campos[1]."
						FROM ".$this->tabla." WHERE eje_id >0 and programa_id > 0 ".$this->filtro."
					)
			as v ORDER BY v.actividad;";
		$res = $this->db->sql_query($sql) or die($this->cadena);
		if($this->db->sql_numrows($res)>0){
			while(list($id,$act,$pon,$tipoAct,$medida,$t1,$t2,$t3,$t4,$t5,$a1,$a2,$a3,$a4,$a5,$avance) = $this->db->sql_fetchrow($res)){
				...
				<div class="panel panel-default">
    				...
    				<div id="collapse'.$idAct.'" class="panel-collapse collapse '.$active.'" role="tabpanel" aria-labelledby="heading'.$idAct.'">
                    	...
                    	<tr>
                        	...
                        	<td class="tdright '.$this->color($tmpDatos['avance']).'"" width="10%">'.$tmpDatos['avance'].'</td>	
	                    </tr>
					</table>';
			if(trim($comentarios) != ""){
