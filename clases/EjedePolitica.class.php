...
class EjedePolitica  extends Comunes implements InterfazCatalogos{
    ...
    function listado(){
        ...
        if($no_registros > 0)
        {
            ...
            $sql="SELECT eje_id,nombre,active,ponderacion FROM cat_ejes WHERE 1 ".$this->cadenaFiltros." ORDER BY eje_id ASC limit ".$this->session['page'].",".$this->session['regs'].";";
            $res=$this->db->sql_query($sql) or die($this->cadena_error);
            if($this->db->sql_numrows($res) > 0)
            {
                $buffer.="<table width='95%' class='table tablesorter table-bordered' align='center' id='MyTableActividades'>
                    <thead>
                        <tr class='alturaTableHeader'>
                            ...
                            <td width='20%' class='tdcenter cabecerasTable'>".PONDERACION."</td>
                    ...
                    <tbody>";
                $c=0;
                while(list($eje_id,$eje,$eje_activo,$pondera) = $this->db->sql_fetchrow($res))
                {
