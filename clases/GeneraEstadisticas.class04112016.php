	...
	function generaCuadroActividades($arrayTotales){
		...
		$this->buffer .='<table width="100%" class="table">
						...
						<tr class="'.$this->color($avanceTotal).'">
							...
							<td class="tdright">Avance Total: '.number_format($arrayTotales['tavance'],2,".",",").'</td>
							<td class="tdright">Promedio Total: '.number_format($avanceTotal,2,".",",").'</td>
						</tr></table>';
			...
			case 1:
				...
				if((int) $this->data['ponderaId'] == 1){
					$buffer .= '<th class="tdcenter" >Actividades Procesadas</th>
								<th class="tdcenter" >Ponderacion</th>';
			...
			case 3:
				...
				case 5:
						...
						<tr>
							...
							<th class="tdcenter" >Actividades</th>';
						if((int) $this->data['ponderaId'] == 1){
							$buffer .= '<th class="tdcenter" >Actividades Procesadas</th><th class="tdcenter" >Ponderacion</th>';
							$buffer .= '<th class="tdcenter" >Promedio</th>';
						}						
						$buffer .= '<th class="tdcenter" >% Avance Acumulado</th>
						</tr>
	...
	function generaCuadroEjeAreaProyecto(){
		...
		if(count($this->arrayDatos) > 0){
			...
			foreach($this->arrayDatos as $ind => $tmp){
				...
				if((int) $this->data['ponderaId'] == 1){
					$this->buffer.= '<td class="tdcenter">'.number_format($tmp['procesadas'],0,'.',',').'</td>
			...
			if((int) $this->data['ponderaId'] == 1){
				$this->buffer.= '<th class="tdcenter">'.number_format($valorPc,0,'.',',').'</th>
		...
		if(count($this->arrayDatos) > 0){
			...
			foreach($this->arrayDatos as $ind => $tmp){
				...
				$this->bufferXml .='<tr>
									...
									<td class="tdcenter">'.number_format($tmp['actividad_id'],0,'.',',').'</td>';
				if((int) $this->data['ponderaId'] == 1){
					$this->bufferXml.= '<td class="tdcenter">'.number_format($tmp['procesadas'],0,'.',',').'</td>
										<td class="tdcenter">'.number_format($tmp['ponderacion'],0,'.',',').'</td>
										<td class="tdcenter">'.number_format($tmp['promedio'],2,'.',',').'</td>';
				}
