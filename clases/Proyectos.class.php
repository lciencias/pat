<?php
class Proyectos  extends Comunes implements InterfazCatalogos{
    var $ddb;
    var $data;
    var $path;
    var $session;
    ...
    function __construct($db,$data,$session,$path,$pages){
        ...
        $this->pages  = $pages;
        $this->tabla  = "cat_subprogramas";
        $this->campo  = "subprograma_id";
        $this->nmBoton="guardarProyecto";
    ...
    function actualizar(){
        ...
        if(count($this->data) > 0)
        {
            ...
            $this->data['subprograma']=$this->data['nombre'];
            $this->data=$this->LimpiaValores($this->data);
            $update  = array();
            foreach($this->data as $clave => $valor)
            {
                if( (trim($clave) != "opc") && (trim($clave) != "random") && (trim($clave) != "idSec") && (trim($clave) != "idarea") && (trim($clave) != "nombre")&& (trim($clave) != "eje_id") )
                {
                    $update[]= $clave." = \"".$valor."\"";
                }
            }
    ...
    function regresaOrden(){
        ...
        AND programa_id='".$this->data['programa_id']."' AND objetivo_id='".$this->data['objetivo_id']."' LIMIT 1;";
        $res=$this->db->sql_query($sql) or die($this->cadena_error);
        if($this->db->sql_numrows($res)>0)
        {
    ...
    function formato(){
        $buffer="
            ...
            <input type='hidden' name='valueId' id='valueId' value='".($this->array_datos['subprograma_id'] + 0)."'>
            <table width='80%' align='center' border='0'>
            <tr>
            <td>".NMPROYECTO."</td>
            <td><input type='text' required='yes' class='form-control validatexto' style='height: 30px;width:300px;' placeholder='".NMPROYECTO."'  value='".$this->array_datos['subprograma']."' maxlength='200' name='nomCatalogo' id='nomCatalogo'>
            ...
            <tr><td>".PROGRAMA."</td><td>".$this->generaProgramas($this->array_datos['area_id'],$this->array_datos['programa_id'],2)."</td></tr>
            <tr><td>".OBJGEN."</td><td>".$this->generaObjetivosGenerales($this->array_datos['area_id'],$this->array_datos['programa_id'],$this->array_datos['objetivo_id'],2)."</td></tr>
    ...
    function generaFiltros(){
        $bufferFiltros="
            <table width='80%' align='center' border='0'>
              ...
              <tr><td class='tdcenter'>".$this->generaProgramas($this->array_datos['area_id'],$this->array_datos['programa_id'],2)."</td></tr>
              <tr><td class='tdcenter'>".$this->generaObjetivosGenerales($this->array_datos['area_id'],$this->array_datos['programa_id'],2)."</td></tr>
    ...
    function recuperaFiltros(){
        ...
        if( trim($this->data['busqNombre'])!= ""){
          $this->cadenaFiltros.=" AND subprograma  like  '".$this->data['busqNombre']."%' ";
        }        
    }
    
    function listado(){
        $arrayA=$this->catalogoAreas();
        $arrayP=$this->catalogoProgramas();
        $arrayO=$this->catalogoObjetivosG();
        $buffer="<div id='resultado' class='error'></div><br>
        <table width='95%' ><tr><td class='tdright'><button class='btn btn-mio' name='buttonBuscar' id='buttonBuscar' type='button' value='1' onclick=\"location='aplicacion.php?aplicacion=".$this->session['aplicacion']."&apli_com=".$this->session['apli_com']."&opc=1'\">".BTNNUEVOPROYECTO."</button></td></tr></table>";
        ...
        if($no_registros > 0)
        {
            ...
            $sql="SELECT ".$this->campo.",subprograma,active,objetivo_id,area_id,programa_id FROM ".$this->tabla."
                ...
                $buffer.="
                    ...
                    <thead>
                        <tr class='alturaTableHeader'>
                            ...
                            <td width='20%' class='tdcenter cabecerasTable'>".PROYECTO."</td>
                            <td width='20%' class='tdcenter cabecerasTable'>".AREA."</td>
                            <td width='20%' class='tdcenter cabecerasTable'>".PROGRAMA."</td>
                            <td width='20%' class='tdcenter cabecerasTable'>".OBJGEN."</td>
                    ...
                    <tbody>";
                $c=0;
                while(list($_id,$_nm,$_act,$objetivo_id,$_areaId,$_programaId) = $this->db->sql_fetchrow($res))
                {
                    ...
                    $buffer.="<tr class=\"row".(($c++%2)+1)."\">
                        ...
                        <td class='tdleft'>".$_nm."</td>
                         <td class='tdcenter'>".$arrayA[$_areaId]."</td>
                         <td class='tdcenter'>".$arrayP[$_programaId]."</td>
                        <td class='tdcenter'>".$arrayO[$objetivo_id]."</td>
