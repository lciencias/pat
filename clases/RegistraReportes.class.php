...
class RegistraReportes extends Comunes {
    ...
    var $visible;
    var $path_sis;
    function __construct($db, $data, $session, $server, $path, $pages,$path_sis) {
        $this->db = $db;
        $this->data = $data;
        $this->path = $path;
    ...
    function listadoProyectos(){
        ...
        $arrayDisabled = $this->recuperaPermisos($this->session['areas'],$this->session['programas'],$this->data['ano_id']);
        $trimestreId   = $this->obtenTrimestre($arrayDisabled);
        if((int)$trimestreId == 0){
        	$trimestreId = $this->regresaTrimestreDefault();
        }
        $noTri = $trimestreId;
        if($trimestreId == 1)
            $noTri ="";
    ...
    function muestraFormularioMetas(){
        ...
        if($this->data['folio']  != ""){
            ...
            $arrayDisabled = $this->recuperaPermisos($arrayProyecto['unidadResponsable_id'],$arrayProyecto['programa_id'],$this->data['ano_id']); 
            $trimestreId   = $this->obtenTrimestre($arrayDisabled);
